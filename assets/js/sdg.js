 ...
 */
 (function($) {
  ...
  // OVERRIDE START - FOR MAP POINTS
  var mapPoints = $('#indicatorData').data('mappoints');
  var pinIcon = L.icon({
    iconUrl: '/assets/img/location-pin.png',
    shadowUrl: '/assets/img/location-pin-shadow.png',
    iconSize:     [40, 40],
    shadowSize:   [10, 10],
    iconAnchor:   [20, 40],
    shadowAnchor: [4, 10],
  ...
  Plugin.prototype = {
    ...
    init: function() {
      ...
      if (mapPoints && mapPoints.length > 0) {
        var pointsURLs = mapPoints.map(function(item) {
          var fileName = item.output,
              filePath = [opensdg.remoteDataBaseUrl, 'geojson', fileName].join('/');
          return $.getJSON(filePath);
        });
        var pointLayerControl = L.control.layers(null, null, {position: 'topleft'}).addTo(plugin.map);
        $.when.apply($, pointsURLs).done(function() {
          ...
          for (var i = 0; i < geoJsons.length; i++) {
            ...
            pointLayerControl.addOverlay(pointLayer, mapPoint.label);
          }
